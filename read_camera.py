...
# RT is pose matrix of camera. it is NOT extrinsic matrix
...
def load_K_Rt_from_P(filename, P=None):
    ...
    K = K/K[2,2]
    intrinsics = np.eye(4) # create 4x4 identity matrix, 1 at diagnol and 0 other place
    intrinsics[:3, :3] = K

    pose = np.eye(4, dtype=np.float32)
    pose[:3, :3] = R.transpose()
    pose[:3,3] = (t[:3] / t[3])[:,0] # normalize

    return intrinsics, pose



root_path = 'data/image_test'
# cam_file = os.path.join(root_path, 'cameras.npz')
cam_file = 'data/cameras.npz'
camera_dict = np.load(cam_file)
n_images = len(camera_dict.files) // 2 # / is true division and // is floor division
...
print('camera params loaded') # Get: intrinsics and poses
