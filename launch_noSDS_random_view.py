...
# so i have tried manymanymany converters, but the first one indeed is the right one......
...
def launch():
    ...
    output_path = 'data/image_output/41_space_6_gl3_50t_1r_newcam_fv'
    cond_space = 6 # set this to 1 to make all the images become condition
    target_space = 6
    translation_weight = 50
    rotation_weight = 1
    intrinsic_all, pose_all, _ = get_camera_poses(cam_file)
    intrinsic = intrinsic_all[0]
    cond_poses, cond_idx = get_cond_camera_poses(pose_all, cond_space)
    target_poses, target_idx = get_target_camera_poses(pose_all, target_space)
